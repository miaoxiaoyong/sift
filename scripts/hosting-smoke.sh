...
cleanup() {
	if [[ -n "${SMOKE_HOME:-}" ]] && [[ -d "$SMOKE_HOME" ]]; then
		if command -v launchctl >/dev/null 2>&1; then
			launchctl bootout "gui/$(id -u)/com.xsift.sift" >/dev/null 2>&1 || \
				launchctl remove com.xsift.sift >/dev/null 2>&1 || true
		fi
...
if [[ "$backend" == "systemd" ]]; then
	...
	plist_pid="$(launchctl list com.xsift.sift 2>/dev/null | awk '/"PID"/{gsub(/[^0-9]/,""); print}' | head -1 || echo "")"
	if [[ -n "$plist_pid" ]] && (( plist_pid > 1 )); then
		...
		deadline=$((SECONDS + 30))
		while (( SECONDS < deadline )); do
			listed="$(launchctl list com.xsift.sift 2>/dev/null || true)"
			if [[ -z "$listed" ]]; then
