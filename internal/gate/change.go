...
func EnqueueMergeChange(ctx context.Context, db *storage.DB, in Input, verdict Verdict, method string, nowMS int64) error {
	if db == nil || in.Identity.RunID == "" || in.Identity.ProjectID == "" || in.Identity.ChangeID == "" || !validSHA(in.Change.HeadSHA) || verdict.Kind != "ready" || verdict.Code != "merge" || verdict.ExpectedHeadSHA != in.Change.HeadSHA || method == "" {
		return errors.New("gate: invalid merge authorization")
	}
	payload, err := json.Marshal(MergeChangePayload{ProjectID: in.Identity.ProjectID, RunID: in.Identity.RunID, ChangeID: in.Identity.ChangeID, ExpectedHeadSHA: in.Change.HeadSHA, Method: method})
	if err != nil {
		return err
	}
