...
macOS 用户按上述归档升级路径执行 `sift service restart` 时保持已安装的 `cn.hexai.sift` launchd label，并清理更早的 `com.miaoxiaoyong.sift` 及其 plist；清理幂等，不会创建重复 agent。无需先额外执行 `sift service install`。
