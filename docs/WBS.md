...
> [Command bootstrap #739 PASS WITH NOTES](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md) / #738（commit `4c2a12e`）落地单一写口 `ApplyCommandEvent`：候选去重 → allowlist 鉴权 → §3.1 字节语法 → 不可变 target/当前 Interrupt/nonce/options 校验 → reason×action 确定性效果，全程同事务复用唯一 transition / `recordHumanDecisionTx` / outbox，且 `EmitInterrupt` 在其五件事事务内创建不可变 command target 绑定。据此勾选八项（管道、唯一 transition、startup_stall options、retry 请求/probe-in-flight、probe 成功 ADR-013 CAS、reject/隔离、M3 仲裁复用、Ledger 语义原料）。留未勾三项：逐项 reason×action 效果（非 startup retry + guardrail/code_review approve 仍 `ErrCommandEffectNotWired`）、`/sift ask` 全契约（agent_blocked 的 Task Spec 快照/终结/新 attempt 未接线）、probe 失败升级计数/达上限 hold（归分离的 AdvanceInterrupt/expiry 路径，未从探测失败端到端验证）。ack 发布 worker 与 probe 进程检查 worker 均不在本片；这不读作 M5 已实现。

- [x] actor 鉴权 → 严格语法 → nonce/current Interrupt/options 校验 → DomainCommand + 回执 outbox（[#739](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md) / #738：`ApplyCommandEvent` 单一写口串联 `lookupCommandReceiptTx` 去重 → `NewAuthorizer` 鉴权 → `ParseCommand` 严格语法 → 不可变 target/current Interrupt/nonce/options 校验 → 效果 + 事件 + receipt + ack outbox。回执 outbox op 已在事务内创建并持久化，但出队发布回 Forge 的 worker 尚未接线——不读作回执已送达）
- [ ] 逐项实现 PRD §7.1 `/sift approve | reject | retry | hold | ask` 与审批标签的 reason-specific 确定性效果；不在当前 options 内一律拒绝（#739/#738 仅 bootstrap：startup_stall 的 retry/reject/hold、design_approval approve、通用 reject/hold/ask 与审批标签 approve 已落地；非 startup retry 与 guardrail/code_review approve 返回 `ErrCommandEffectNotWired` 留后续波次；“不在当前 options 内一律拒绝”已由 `Compile` optionAllowed 实现并测试）
- [ ] `/sift ask <文本>` 同事务写当前 Run 的任务层澄清与 Ledger 语义原料，并按当前 Interrupt 契约继续；不得自动升格项目/全局 Context（#739/#738：`commandAskTx` 同事务写 `DecisionAsk` + `SemanticMaterial` 并 close/responded、不升格 Context——与末条 Ledger 语义原料重叠已验；但 agent_blocked 的完整 ask 效果——Task Spec 快照 + 绑定 attempt 终结 + 下一个 attempt/claim/launch——未接线，留后续波次）
...
- [ ] 探测失败复用同一 Interrupt、升级计数、轮换 nonce；达到上限 hold（#739/#738：`probeFailedTx` 复用 Interrupt、旋转 nonce、置 `dispatch_state=held` 并发 `absence_unconfirmed` ack，`TestApplyRetryProbeResultFailureAbsenceUnconfirmed` 验证；但升级计数与达上限 hold 归分离的 AdvanceInterrupt/expiry 路径（[#711](reviews/2026-07-30-m5-advance-interrupt-711-rereview-pi-deepseek-v4-pro.md)/[#727](reviews/2026-07-30-m5-advance-interrupt-727-rereview-pi-deepseek-v4-pro.md)），未从探测失败端到端验证；probe 进程检查 worker 不在本片——复审注记 3/4）
