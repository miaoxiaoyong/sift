...
> M5 波次二 Command bootstrap（Issue #737 / PR #738，复审 [#739 PASS WITH NOTES](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)）已闭合 `startup_stall` 两段式的 Command 写入端口：鉴权/语法/nonce/options 校验、唯一 transition + outbox、retry 请求/结果两段式与 ADR-013 单一 CAS、reject/ask 语义原料与 marker 契约。非阻断注记：`command_ack` outbox 写入已证据但 Forge 发布 worker 未接线；probe finalizer 端口已测、probe worker 未接线；非 startup retry 与 guardrail/code_review approve 仍返 `ErrCommandEffectNotWired`，故 PRD §7.1 全 reason-specific 效果（第 2 项）保持未勾。critical 熔断、Report、预算、指标与 M5 门禁整体仍开，不读作 M5 已实现。

- [x] actor 鉴权 → 严格语法 → nonce/current Interrupt/options 校验 → DomainCommand + 回执 outbox（[#739 #1](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`ApplyCommandEvent` 单一写入口径串联 `lookupCommandReceiptTx` 去重 → `NewAuthorizer`（不可变 config 快照）→ `ParseCommand` 严格 ABNF → `resolveCommandInterruptTx` 不可变 target 绑定 → `Compile` nonce/cutoff/options/label position → effect → `writeAcceptedCommandReceiptTx` + `writeCommandAckOpTx` outbox。`command_ack` outbox 写入已证据，Forge 发布 worker 未接线属非阻断注记，仍可勾选本项）
- [ ] 逐项实现 PRD §7.1 `/sift approve | reject | retry | hold | ask` 与审批标签的 reason-specific 确定性效果；不在当前 options 内一律拒绝（[#739 #2 备考](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`startup_stall` 的 options 约束与 reject/hold/ask/approve 标签已实现，但非 startup retry（failure_review/agent_blocked/merge_conflict）与 guardrail/code_review approve 仍返 `ErrCommandEffectNotWired`、Gate re-evaluation operation 与 attempt terminalization 未接线，全 reason-specific 效果未闭合，本项保持未勾）
- [x] `/sift ask <文本>` 同事务写当前 Run 的任务层澄清与 Ledger 语义原料，并按当前 Interrupt 契约继续；不得自动升格项目/全局 Context（[#739 #9](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`commandAskTx` → `recordHumanDecisionTx`（`DecisionAsk` + `SemanticMaterial: c.AskText`），`TestApplyCommandAskWritesSemanticMaterial` 验证 ask 文本持久化；Command 效果仅落当前 Run/Interrupt 事务，无项目/全局 Context 写路径）
- [x] 通用 reject/retry/hold/approve 均经唯一 transition 与 outbox，不直接写状态；标签和评论路径共用鉴权/幂等实现（[#739 #7/#10](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：所有动作经 `d.transition()` + `recordHumanDecisionTx`，ack 统一 `writeCommandAckOpTx`/`writeProbeAckOpTx`，标签 approve 与评论 approve 同路径，去重口诀 `lookupCommandReceiptTx` 检 `(project_id,event_kind,remote_event_id)`；`command.go`/`command_probe.go` 无绕过核心的直写 SQL）
- [x] `startup_stall` 只提供 retry/reject/hold，approve 必须拒绝（[#739 #2](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`Compile` 中 `optionAllowed` 拒绝不在 options 内的动作，`EmitInterrupt` 只为 startup_stall reason 设 `options=["retry","reject","hold"]`；`TestCompileStartupStallApproveRejected` + `TestApplyCommandStartupStallApproveRejectedOption` 验证 approve → `rejected_option`）
- [x] retry 请求不关闭 Interrupt、不写 resolution；探测在途拒绝新指令并回复“已在探测中”（[#739 #3](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`startupStallRetryRequestTx` CAS 轮换 nonce + `dispatch_state=probe_in_progress`、写 pending `retry_pending` event/`attempt_probes`/HumanDecision，不关 Interrupt、不写 `attempt_resolution`、不建 ack；`Compile` 检 `DispatchProbeInProgress` → `OutcomeProbeInProgress`；`TestApplyCommandStartupStallRetryRequestPendingNoAck` + `TestApplyCommandStartupStallProbeInProgressRejects` 验证）
- [x] 探测失败复用同一 Interrupt、升级计数、轮换 nonce；达到上限 hold（[#739 #4](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`probeFailedTx` 标 probe failed → 轮换 nonce + `dispatch_state=held` → `finalizeRetryOutcomeTx`（`absence_unconfirmed` final + pending→final CAS）→ `writeProbeAckOpTx`，Interrupt 保 open、isolation 保 frozen；`TestApplyRetryProbeResultFailureAbsenceUnconfirmed` 验证。At-cap hold 走现有 `AdvanceInterrupt` + `OnMaxEscalations: ExpireHold`，非本 slice 新增）
- [x] 探测成功以 ADR-013 单一 CAS 事务提交：消失证据、旧 attempt 终结、`retry_after_absence`、解除隔离、关闭 Interrupt、Run → queued、新 attempt/claim、启动与回执 operation、事件（[#739 #5](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`probeSucceededTx` 单事务内 probe→succeeded + evidence digest + `attempt_resolution=retry_after_absence` + `closeInterruptTx` responded + transition waiting_human→queued + `finalizeRetryOutcomeTx` applied + `isolation_state=none` + `isolation_release_event_id` + `writeProbeAckOpTx`，任一 CAS 失败回滚；`TestApplyRetryProbeResultSuccessClosesAndQueues` 验证 7 步全或全无。后续 attempt/claim/launch 经现有 queued launch 路径，probe worker 未接线）
- [x] reject 写 `attempt_resolution=reject`、Run failed，但隔离保持到执行体被证明消失（[#739 #6](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`commandRejectTx` transition → RunFailed(human_reject) → `attempt_resolution=reject`（NULL→reject write-once）→ `recordHumanDecisionTx` reject + 语义原料 → `closeInterruptTx` responded，isolation 保 frozen；`TestApplyCommandStartupStallRejectFailsRunHoldsIsolation` 验证）
- [x] 所有动作复用 M3 唯一仲裁函数；覆盖事实先到/决定先到/探测在途事实到达（[#739 #8](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：Command 代码不直接调 `ResolveAttemptRace`，但按 ADR-013 设 `attempt_resolution` marker（reject→`reject`、probe success→`retry_after_absence`、retry request 不写 marker 保持事实窗口），M3 race 检 marker 决定 `superseded_by_decision`/`superseded_by_fact`/`running`，三场景（事实先到 CAS 拒、决定先到 marker 已写、探测在途 marker 未写）覆盖）
- [x] `/sift reject` 原因与 `/sift ask` 文本随人类决定写入 Ledger，不丢语义原料（[#739 #9](reviews/2026-07-30-m5-command-bootstrap-739-rereview-pi-deepseek-v4-pro.md)：`commandRejectTx` → `recordHumanDecisionTx`（`DecisionReject` + `SemanticMaterial: c.RejectReason`），`commandAskTx` → `recordHumanDecisionTx`（`DecisionAsk` + `SemanticMaterial: c.AskText`）；`TestApplyCommandReject` 验证 1 条 `semantic_material`，`TestApplyCommandAskWritesSemanticMaterial` 验证 ask 文本持久化）
