...
| V2-14 | 人工态 started/result 与决定的单事务仲裁 | `TestV4HumanStateInterleavings`、`TestV4HumanStateInterleavingsIncludeLateResults`、`TestV4HumanStateInterleavingsNoInitialInterrupt`、`TestResolveAttemptRaceProbeStartedFactWinsInvalidates`、`TestResolveAttemptRace*` | covered #851 | 两 backend 交错调用图 |
...
| X10 | kill 无 successor；retry absence 后仅一个 successor | `TestTerminationKillAfterAbsenceFailsWithoutNewAttempt`、`TestTerminationRetryAfterAbsenceCreatesNewAttempt`、`TestProbeTickObserveAbsentDrivesSuccessArm`、`TestV4KillRetryBackends/{process,tmux}/*` | covered #851 双 backend/并发 |
| X11 | Interrupt commit 前/后 × started，decision 前/后 × started | `TestV4HumanStateInterleavings`、`TestV4HumanStateInterleavingsIncludeLateResults`、`TestV4HumanStateInterleavingsNoInitialInterrupt` 四格 storage seam | covered #851 两 backend生产调用图 |
| X12 | X11 的 late `result.json` 对称重放 | `TestV4HumanStateInterleavingsIncludeLateResults`、`TestV4HumanStateInterleavingsNoInitialInterrupt`、`TestResolveAttemptRacePersistsLateResult` | covered #851 |
...
| X15 | probe in flight 时合法 started fact wins + invalidation ack | `TestResolveAttemptRaceProbeStartedFactWinsInvalidates` | covered #851 |
