...
| 层 | 载体 | 内容 |
|----|------|------|
| 单元 | `internal/hosting` | 后端按 GOOS 分发；spec 解析（已装 release、拒绝相对 home / 不可执行 daemon / 非 SemVer current）；launchd/systemd 模板字段（KeepAlive、Restart=on-failure、ExecStart=current/sift daemon、SIFT_HOME、绝对日志路径）；launchd 固定 PATH（`/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`）而非 shell PATH；**含 `&` / 空格 / `#` 的 SIFT_HOME 渲染两后端，XML 实体 / systemd 引号转义后经 `plutil -lint`（darwin）/ `systemd-analyze --user verify`（linux，可用时）校验且解析出的 ExecStart/SIFT_HOME 与原路径一致（`TestRenderEscapesUserPathsInUnits`）**；foreground 无单元；launchd install plan 在 fresh/current-loaded 两态均 `bootout` 后 `bootstrap`，仅 fresh 的 `No such process` 可忽略，bootstrap 失败必须传播；**install 的 teardown quiescence（Issue #968）：bootout 成功后以 `print` 有界等待 absent（注入 clock/sleeper + 可缩小窗口，毫秒级确定性），确认 absent 才 bootstrap；瞬时 exit 5 且缺席仍确认时有界重试，重试前重探 absence；权限 / no-GUI domain / malformed / ambiguous-or-present 一律不重试；耗尽返回可操作错误（`TestLaunchdInstallImmediateSuccess/DelayedDisappearance/TransientBootstrapThenSuccess/RetryExhaustion/TeardownTimeoutNeverBootstraps/PermissionErrorDoesNotRetry/DomainErrorDoesNotRetry/AmbiguousStateDoesNotRetry`，脚本化 fake launchctl 按调用计数驱动）**；**launchd start 对 loaded label kickstart、unloaded retained plist bootstrap，start 不写 plist；kickstart/bootstrap 权限及其它非零失败传播，SSH/no-GUI domain 输出前台/登录 GUI 提示**；**foreground status 实探 `siftd.sock` 并携带 `present|absent` 两态判定 + socket 路径（`TestPlanStatusForegroundProbesSocket`）**；原子写（temp+rename，无残留 staging）；Exec 在工具缺失时返回 `ErrNoBackend`；formula 草稿与归档名一致、装两二进制、指向 service install、no-port |
...
13. **install teardown quiescence 序列（Issue #968）**：`sift service install` 在 darwin 上经 CLI 全链路断言 `bootout → print(absent) → bootstrap` 顺序，且瞬时 exit 5 后重试成功（`TestServiceInstallLaunchdReplacesFreshAndCurrentUnit` / `TestServiceInstallLaunchdRetriesTransientBootstrap`）；hosting 层以脚本化 fake launchctl（按调用计数驱动）+ 注入 sleeper/时钟覆盖六种必测序列 + 两种边界（见金字塔）。
14. **smoke 失败条件（R1 P1-3）**：`hosting-smoke.sh` 对「daemon 未启动（foreground 立即退出 / supervised wait 超时）、kill 后 supervisor 未产出新 PID（systemd）/ 未再注册（launchd）、restart 后新 release 未运行（marker 缺失）」均 exit 非 0；`hosting-smoke-test.sh` 以 fake supervisor 对每个负例断言非 0、对 happy 路径断言 0。
...
smoke 脚本在无 user manager 的 CI runner（如默认 GitHub Actions ubuntu）上会落回 foreground 路径并如实报告「autorestart not promised in this mode」；systemd 自启需 `XDG_RUNTIME_DIR` + `systemctl --user is-system-running` 可达，A10 干净机（M7 后）才是完整证据。harness 的 systemd 场景需在 Linux 上跑（CLI backend 跟随 runtime.GOOS），launchd 场景需在 macOS 上跑；负例在对应平台均非 0。harness 的 fake launchctl 忠实模拟当前 install 序列：`bootstrap`/`load` 启动 daemon（RunAtLoad 语义）、`print` 回答 install 的缺席探针（#968）、live-listener 门用 `sift doctor --json` 的 `"ok"` 信封判定（stale socket 无 listener 必拒）。
