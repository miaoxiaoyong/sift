...
| 字段 | 值 | 理由 |
|------|----|------|
| `Label` | `cn.hexai.sift` | v0.5.4 起已安装的稳定句柄；仓库身份迁至 xsift 不改变运行时 service identity；`launchctl kickstart -k gui/<uid>/<Label>` 原子重启 |
...
2. macOS 用户执行 `sift service restart`（或 `sift service install`）时保持已安装的 `cn.hexai.sift` launchd label；仅清理更早的 `com.miaoxiaoyong.sift` agent 及其 plist。旧 label 不存在时该步骤幂等，因此不会留下重复 agent。
