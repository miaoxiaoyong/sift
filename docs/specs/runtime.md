...
`agent_definition_hash` 是 closed canonical `{"schema_version":1,"args":[],"task_transport":"stdin|file","version_args":[]}` 的 SHA-256；不含 backend/max concurrency，因为两个 backend 必须共享 wrapper→Agent 拓扑。`executable_sha256` 对 symlink-resolved regular file bytes 计算。版本命令只经 argv 执行且必须 exit 0；`version_output_digest = SHA256(stdout_bytes || 0x00 || stderr_bytes)`，不 trim/改码，原始输出不进入普通事件。digest 与 executable bytes 防止同路径替换后继承旧资格。
